...
class Modchart
{
	...
	static function toLua(l:State, val:Any):Bool 
    {
		switch (Type.typeof(val)) 
        {
			...
			case Type.ValueType.TObject:
				objectToLua(l, val);
			default:
				trace(val + " is not supported. (Type: " + Type.typeof(val) + ")");
				return false;
		}
    ...
    function new()
    {
        ...
        result = LuaL.dofile(lua, Paths.lua(songLowercase + "/modchart"));
		...
		setVar("showP1Strums", true);
		setVar("showP2Strums", true);
    ...
    public static function createModchartState(type:String, typeName = null):Modchart
    {
        var modchart = new Modchart();
		modchart.type = type;
