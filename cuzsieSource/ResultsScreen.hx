...
class ResultsScreen extends FlxSubState
{
	...
	{	
		...
		var winThing:FlxSprite = new FlxSprite(0,0).loadGraphic(Paths.image("ui/Win", 'shared'));// WIN WIN WIN WIN WINW INW INW INW IWN IWN IWN IWN IWN WI NIWWN - gordon freakman
        add(winThing);
        ...
        settingsText = new FlxText(20,FlxG.height + 50,0,'SF: ${PlayState.rep.replay.sf} | Ratio (SA/GA): ${Math.round(sicks)}:1 ${Math.round(goods)}:1 | Mean: ${mean}ms | Played on ${PlayState.SONG.song} ${Utility.difficultyFromInt(PlayState.storyDifficulty).toUpperCase()}');
