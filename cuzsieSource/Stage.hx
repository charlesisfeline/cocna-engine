...
// el kade engine 1.7 momento
class Stage
{
    public var curStage:String = ''; // Current active stage
    public var halloweenLevel:Bool = false; // If the lightning strike thingy is active
    public var camZoom:Float; // The zoom of the camera
    ...
    public function new(daStage:String)
    {
        ...
        switch(daStage)
        {
			...
			default:
			{
				...
				var bg:FlxSprite = new FlxSprite(-600, -200).loadGraphic(Paths.stages('stage/stageback'));
				bg.antialiasing = true;
				bg.scrollFactor.set(0.9, 0.9);
				bg.active = false;

				var stageFront:FlxSprite = new FlxSprite(-650, 600).loadGraphic(Paths.stages('stage/stagefront'));
				...
				var stageCurtains:FlxSprite = new FlxSprite(-500, -300).loadGraphic(Paths.stages('stage/stagecurtains'));
            ...
            case 'halloween':
				...
				case 'limo':
					{
						...
						var skyBG:FlxSprite = new FlxSprite(-120, -50).loadGraphic(Paths.stages('limo/limoSunset'));
						...
						trace(Paths.getSparrowAtlas('limo/bgLimo', 'stages'));
						bgLimo.frames = Paths.getSparrowAtlas('limo/bgLimo', 'stages');
						...
						var limoTex = Paths.getSparrowAtlas('limo/limoDrive', 'stages');
				...
				case 'mall':
					{
						...
						var bg:FlxSprite = new FlxSprite(-1000, -500).loadGraphic(Paths.stages('mall/bgWalls'));
						...
						upperBoppers.frames = Paths.getSparrowAtlas('mall/upperBop', 'stages');
						...
						var bgEscalator:FlxSprite = new FlxSprite(-1100, -600).loadGraphic(Paths.stages('mall/bgEscalator'));
						...
						var tree:FlxSprite = new FlxSprite(370, -250).loadGraphic(Paths.stages('mall/christmasTree'));
						...
						bottomBoppers.frames = Paths.getSparrowAtlas('mall/bottomBop', 'stages');
						...
						var fgSnow:FlxSprite = new FlxSprite(-600, 700).loadGraphic(Paths.stages('mall/fgSnow'));
						fgSnow.active = false;
						...
						santa.frames = Paths.getSparrowAtlas('mall/santa', 'stages');
				...
				case 'mallEvil':
					{
						var bg:FlxSprite = new FlxSprite(-400, -500).loadGraphic(Paths.stages('mallEvil/evilBG'));
						bg.antialiasing = FlxG.save.data.antialiasing;
						bg.scrollFactor.set(0.2, 0.2);
						bg.active = false;
						bg.setGraphicSize(Std.int(bg.width * 0.8));
						...
						var evilTree:FlxSprite = new FlxSprite(300, -300).loadGraphic(Paths.stages('mallEvil/evilTree'));
						evilTree.antialiasing = FlxG.save.data.antialiasing;
						evilTree.scrollFactor.set(0.2, 0.2);
						swagBacks['evilTree'] = evilTree;
                        toAdd.push(evilTree);

						var evilSnow:FlxSprite = new FlxSprite(-200, 700).loadGraphic(Paths.stages("mallEvil/evilSnow"));
				...
				case 'school':
					{
						...
						var bgSky = new FlxSprite().loadGraphic(Paths.stages('school/weebSky'));
						...
						var bgSchool:FlxSprite = new FlxSprite(repositionShit, 0).loadGraphic(Paths.stages('school/weebSchool'));
						bgSchool.scrollFactor.set(0.6, 0.90);
						swagBacks['bgSchool'] = bgSchool;
                        toAdd.push(bgSchool);

						var bgStreet:FlxSprite = new FlxSprite(repositionShit).loadGraphic(Paths.stages('school/weebStreet'));
						bgStreet.scrollFactor.set(0.95, 0.95);
						swagBacks['bgStreet'] = bgStreet;
                        toAdd.push(bgStreet);

						var fgTrees:FlxSprite = new FlxSprite(repositionShit + 170, 130).loadGraphic(Paths.stages('school/weebTreesBack'));
						...
						treeLeaves.frames = Paths.getSparrowAtlas('school/petals', 'stages');
				...
				case 'schoolEvil':
					{
						...
						bg.frames = Paths.getSparrowAtlas('schoolEvil/animatedEvilSchool', 'stages');
	...
	public static function getStageColors(stage:String)
	{
		var poop:Array<String> = Utility.coolTextFile("assets/stages/" + stage + "/data.txt"); // grab color string from file
		var butthole:FlxColor = Utility.getColorString(poop[0]); // convert string to flxcolor
		return butthole;
	}
}