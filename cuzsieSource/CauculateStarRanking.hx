...
class CauculateStarRanking
{
	...
	{
		// cleaned notes
		var cleanedNotes:Array<SimpleNoteData> = [];
		...
		for (i in song.notes) // sections
		{
			for (ii in i.sectionNotes) // notes
			{
				...
				if (ii[1] >= 3 && gottaHitNote)
					cleanedNotes.push(new SimpleNoteData(ii[0] / FreeplayState.rate, Math.floor(Math.abs(ii[1]))));
		...
		var handOne:Array<SimpleNoteData> = [];
		var handTwo:Array<SimpleNoteData> = [];
		...
		for (i in 0...segmentsOne.length)
			segmentsOne[i] = new Array<SimpleNoteData>();
		for (i in 0...segmentsTwo.length)
			segmentsTwo[i] = new Array<SimpleNoteData>();
		...
		for (i in 0...segmentsOne.length)
		{
			...
			if (ve == null)
				continue;
			var fuckYouOne:Array<SimpleNoteData> = [];
			var fuckYouTwo:Array<SimpleNoteData> = [];
			for (note in ve)
			{
	...
	public static function fingieCalc(floats:Array<SimpleNoteData>, columArray:Array<Float>):Float
	{
		var sum:Float = 0;
		if (floats.length == 0)
			return 0;
